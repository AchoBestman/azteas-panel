<?php if(Registry::get('username')) { ?>
       ...
       <div id="menu" class="row w-100">
        ...
        <?php
          // Azteas : masque le lien "search" quand on est deja sur search.php
          // (raccourci redondant vers la page courante).
          $azteas_on_search_page = (basename($_SERVER['SCRIPT_NAME']) == 'search.php');
        ?>
        <?php if(!$admin_user && !$azteas_on_search_page) { ?>
        ...
        <?php } ?>
         ...
         <div class="col">
          ...
          <?php } ?>
           <!-- Azteas : bouton de deconnexion plus explicite (icone + texte visible,
                mis en evidence) plutot qu'une simple icone sans texte. -->
           <a href="<?php print PATH_PREFIX; ?>logout.php" class="menulink logout-link" title="<?php print $text_logout; ?>"><i class="bi bi-box-arrow-right"></i>&nbsp;<?php print $text_logout; ?></a>
          </div>
         </div>
       </div>
